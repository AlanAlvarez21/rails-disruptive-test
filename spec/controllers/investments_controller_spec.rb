...
RSpec.describe InvestmentsController, type: :controller do
  describe 'POST #calculate' do
    context 'when CSV file exists' do
      let(:csv_file) { fixture_file_upload('./fixtures/files/origen.csv', 'text/csv') }
    ...
    context 'when CSV file does not exist' do
      ...
      it 'returns error message' do
        post :calculate
        expect(JSON.parse(response.body)['error']).to eq('No such file or directory @ rb_sysopen - origen.csv')
      end
    end
  end
  ...
  describe 'POST #recaulculate_roi' do
    ...
    context 'when CSV file exists' do
      let(:csv_file) { fixture_file_upload('files/origen.csv', 'text/csv') }
